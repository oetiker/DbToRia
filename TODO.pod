...
The set of available filter operators depends on the choice of database
backend.
...
=head3 Configurable Toolbar content
...
buttons are configurable from the server side.

Status: available,
Effort: 4h

=head3 AutoForm with setable content
...
=head3 Gedafe Friendly Names Cache
...
=head3 Default Values in New Records
...
=head3 Record Switching in Edit Form.

Extend the Edit form with MS-Access-like navigation buttons at the bottom
to allow switching between records without closing the Edit form.
...
Linux: dbtoria.rpm and dbtoria.deb packages.

=item

Windows: dbtoria.exe installer.
