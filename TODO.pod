...
of toolbar buttons for directly accessing tables, view or reports.

=over

=item

Gedafe-compatible toolbar configuration based on table naming scheme.

=item

The buttons are configurable on a per-application base from the server
side.
...
=head3 Copy Values in New Records

The next New Record is populated with values from the previous record
as defined by meta fields.
